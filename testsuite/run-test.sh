...
if [ -x "${PBUILDER_UML}" ]; then
	...
	pbuilder-user-mode-linux create --mirror http://ftp.jp.debian.org/debian/ --distribution "${distribution}" --uml-image $(pwd)/testimage --logfile pbuilder-user-mode-linux-create-${distribution}.log
	( 
	    mkdir testbuild
	    cd testbuild
